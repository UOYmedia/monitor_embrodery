 ...
 *
 * Tạm thời xưởng chỉ cần Tổng quan đội máy, nên bốn tab kia tắt: bớt chỗ bấm nhầm, header
 * ngắn lại một dòng. Code của chúng còn nguyên — bật lại chỉ là thêm tên vào mảng này, không
 * phải viết lại gì. Tab bị tắt mà ai đó còn giữ link cũ (`?tab=audit`) thì `decodeView` đưa
 * về Tổng quan chứ không mở một màn hình không có đường quay ra.
 *
 * Bảng andon treo TV vẫn chạy: nó là địa chỉ riêng `?andon=1`, không phải một tab.
 */
export const tabs: Tab[] = ['fleet']
