...
export function MachineDetail({
  ...
  }, [api, identity.id, identity.updatedAt])
  ...
  return (
    <aside className="detail" aria-label={`Chi tiết máy ${identity.name}`}>
      ...
      {tab === 'production' && <MachineProduction api={api} machine={machine} site={site} timeZone={timeZone} />}
...
function MachineProduction({ api, machine, site, timeZone }: {
  api: BridgeApi
  machine: MachineView
  site: Site | null
  timeZone?: string
  ...
  return (
    <section className="detail-block">
      ...
      {report && report.rows.length === 0 && (
        <p className="muted">Chưa có dòng sản lượng nào trong 7 ngày qua. Máy phải đọc được bộ đếm mũi thì mới có số.</p>
      )}
      {report && report.rows.length > 0 && (
        <>
          <p className="reading-meta">
            Tổng {formatNumber(report.totals.stitches)} mũi · {formatHours(report.totals.runSeconds)} chạy máy ·{' '}
