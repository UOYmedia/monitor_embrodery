...
/** Một lượt đọc do người gõ. `observedAt` là lúc ĐỌC màn hình, không phải lúc gõ vào form. */
...
/**
 * Bridge trả về đủ thứ để màn hình nói thật được, không chỉ "đã lưu".
 *
 * `impliedStitchesPerMinute` là thứ duy nhất phát hiện được một con số sai mà vẫn nằm trong giới
 * hạn vật lý của máy — thừa một chữ số sau một khoảng nghỉ dài thì bridge không bắt được, nhưng
 * "1.400 mũi/phút suốt 8 tiếng" thì người đứng máy nhìn là biết sai.
 * `persisted: false` nghĩa là con số đang chỉ nằm trong RAM của bridge.
 */
export interface ManualReadingResult {
  ...
  reading: {
    ...
    counterReset: boolean
    baseline: boolean
    previous: { odometer: number; at: string; quality: string } | null
    delta: number | null
    elapsedSeconds: number | null
    impliedStitchesPerMinute: number | null
  }
  counted: { counted: boolean; reason: string; stitches?: number; lane?: string; bucketKey?: string }
  persisted: boolean
}
...
export class BridgeApi {
  ...
  /**
   * Ghi một lượt đọc bộ đếm do người gõ.
   *
   * Không có đường sửa hay xoá: một lượt đọc đã ghi là một lời khai có tên người và có giờ, và
   * cách chữa một con số sai là gõ tiếp một con số đúng, không phải sửa lại lời khai cũ.
   */
  manualReading(id: string, input: ManualReadingInput) {
    return this.request<ManualReadingResult>('POST', `/machines/${encodeURIComponent(id)}/manual-reading`, input)
  }
