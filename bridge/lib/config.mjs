...
function mergeNumbers(defaults, configured = {}, label) {
  if (configured && typeof configured !== 'object') fail(`${label} phải là object.`)
  const merged = { ...defaults }
  for (const [key, fallback] of Object.entries(defaults)) {
    if (typeof fallback === 'boolean') {
      merged[key] = configured?.[key] === undefined ? fallback : Boolean(configured[key])
...
function normalizeIngest(raw, base) {
  ...
  const ingest = {
    enabled: configured.enabled === undefined ? defaultIngestConfig.enabled : Boolean(configured.enabled),
    ...
    capture: configured.capture === undefined ? defaultIngestConfig.capture : Boolean(configured.capture),
...
export async function loadConfig(configPath) {
  ...
  return {
    ...
    logLevel: ['debug', 'info', 'warn', 'error'].includes(raw.logLevel) ? raw.logLevel : defaultConfig.logLevel,
    sites,
    freshness,
