 ...
 */
export function reading(value, { observedAt, source }) {
  return value === null || value === undefined ? null : { value, observedAt, source, quality: 'verified' }
}
 ...
 */
export function normalizeTelemetry(raw, { machine, receivedAt = new Date().toISOString(), source }) {
  ...
  const context = { observedAt, source: source ?? machine.adapter }
