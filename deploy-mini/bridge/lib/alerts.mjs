...
export function deriveAlerts(record, telemetry, maintenance, { threadBreakWarnPer1000 = null } = {}) {
  ...
  for (const event of telemetry?.events ?? []) {
    ...
    // Nguồn đi theo sự kiện, không đóng cứng ở đây. Một node cảm biến chỉ thấy trục ngừng quay
    // chứ không biết vì sao; in nhãn "Controller báo" lên phán đoán đó là bảo thợ rằng máy đã tự
    // khai, và thợ sẽ mở máy tìm một lỗi mà controller chưa hề báo.
    const fromSensor = event.source === 'sensor'
    alerts.push({
      id: `event:${event.id}`,
      severity: event.severity,
      kind: fromSensor ? 'sensor-event' : 'controller-event',
      title: event.message || `Mã ${event.code} từ ${fromSensor ? 'node cảm biến' : 'controller'}.`,
      detail: `Mã ${event.code}${event.needle === null || event.needle === undefined ? '' : ` · kim #${event.needle}`}`,
      source: event.source,
  ...
  if (window && window.stitches > 0 && threadBreakWarnPer1000 !== null) {
    ...
    if (rate > threadBreakWarnPer1000) {
      alerts.push({
        ...
        title: `Nguy cơ đứt chỉ: ${rate.toFixed(1)} lần / 1.000 mũi${window.needle === null || window.needle === undefined ? '' : ` ở kim #${window.needle}`}.`,
