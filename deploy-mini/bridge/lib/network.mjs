...
export async function scanSubnet({ cidr, ports, sites, siteId, timeoutMs = 350, concurrency = 12, maxHosts = 256, maxPorts = 12, safety = {}, signal = null }) {
  ...
  const results = []
  let cursor = 0
  // Đếm host THẬT SỰ đã dò, không phải số host định dò. Bấm huỷ một lượt /24 rồi báo
  // "đã quét 254 host, tìm thấy 0" là biến "chưa quét được" thành "đã quét, không có gì" —
  // đúng kiểu bịa dữ liệu mà repo cấm, và nó còn đi thẳng vào nhật ký kiểm toán.
  let daDo = 0
  ...
  return { siteId: site.id, cidr: normalizedCidr, hostsScanned: daDo, hostsPlanned: hosts.length, aborted: Boolean(signal?.aborted), portsScanned: requestedPorts, results: results.sort((a, b) => toInt(a.ipAddress) - toInt(b.ipAddress)) }
}
