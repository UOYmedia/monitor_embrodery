...
describe('ProductionLog.record — làn số nhập tay', () => {
  const manual = (odometer, observedAt, { status = 'unknown' } = {}) => ({
    machineId: machine.id,
    observedAt,
    odometer: { value: odometer, observedAt, source: 'manual:ketoan', quality: 'manual' },
    status: { value: status, observedAt, source: 'manual:ketoan', quality: 'manual' },
  })

  it('ghi vào manualStitches, không vào stitches', () => {
    const log = newLog()
    log.record(manual(1_000_000, '2026-08-14T03:00:00Z'), { machine, site })
    log.record(manual(1_012_000, '2026-08-14T03:15:00Z'), { machine, site })
    const [bucket] = log.query()
    // `manualReadings` là 1, không phải 2: lượt đầu là mốc, và mốc thì chưa mở sổ ca nào — cùng
    // đúng luật với làn máy, nơi lượt poll đầu sau khi khởi động cũng không được tính là lượt đọc.
    expect(bucket).toMatchObject({ stitches: 0, manualStitches: 12_000, readings: 0, manualReadings: 1 })
  })

  it('không cộng runSeconds dù người gõ khai máy đang chạy', () => {
    const log = newLog()
    log.record(manual(1_000_000, '2026-08-14T03:00:00Z', { status: 'running' }), { machine, site })
    log.record(manual(1_012_000, '2026-08-14T03:15:00Z', { status: 'running' }), { machine, site })
    // Một người lấy mẫu một khoảnh khắc không chứng minh được máy chạy suốt 15 phút, mà runSeconds
    // là thứ hiệu suất được tính từ đó.
    expect(log.query()[0].runSeconds).toBe(0)
  })

  it('dùng CHUNG con trỏ với làn máy, nên cùng một khoảng mũi không vào sổ hai lần', () => {
    const log = newLog()
    log.record(snapshot(1_000_000, '2026-08-14T03:00:00Z'), { machine, site })
    log.record(manual(1_010_000, '2026-08-14T03:10:00Z'), { machine, site })
    log.record(snapshot(1_015_000, '2026-08-14T03:20:00Z'), { machine, site })
    const [bucket] = log.query()
    // 10 000 mũi thuộc làn tay, 5 000 thuộc làn máy. Tổng đúng 15 000, không phải 25 000.
    expect(bucket).toMatchObject({ stitches: 5_000, manualStitches: 10_000 })
    expect(bucket.stitches + bucket.manualStitches).toBe(15_000)
  })

  it('khoảng nào được đóng bởi lượt đọc nào thì thuộc làn đó, trọn khoảng', () => {
    const log = newLog()
    log.record(snapshot(1_000_000, '2026-08-14T03:00:00Z'), { machine, site })
    const result = log.record(manual(1_009_000, '2026-08-14T03:09:00Z'), { machine, site })
    expect(result).toMatchObject({ counted: true, stitches: 9_000, lane: 'manual' })
    expect(log.query()[0]).toMatchObject({ stitches: 0, manualStitches: 9_000 })
  })

  it('bộ đếm lùi vẫn ghi cờ resets, bất kể ai khai', () => {
    const log = newLog()
    log.record(manual(1_000_000, '2026-08-14T03:00:00Z'), { machine, site })
    const result = log.record(manual(500, '2026-08-14T03:05:00Z'), { machine, site })
    expect(result).toMatchObject({ counted: false, reason: 'counter-reset', lane: 'manual' })
    expect(log.query()[0]).toMatchObject({ resets: 1, manualStitches: 0 })
  })

  it('con trỏ mang theo loại của lượt đọc gần nhất', () => {
    const log = newLog()
    log.record(snapshot(1_000_000, '2026-08-14T03:00:00Z'), { machine, site })
    expect(log.cursorFor(machine.id)).toMatchObject({ odometer: 1_000_000, quality: 'verified' })
    log.record(manual(1_005_000, '2026-08-14T03:05:00Z'), { machine, site })
    expect(log.cursorFor(machine.id)).toMatchObject({ odometer: 1_005_000, quality: 'manual' })
  })

  it('cursorFor trả bản sao và trả null khi chưa có mốc', () => {
    const log = newLog()
    expect(log.cursorFor(machine.id)).toBeNull()
    log.record(snapshot(1_000, '2026-08-14T03:00:00Z'), { machine, site })
    const cursor = log.cursorFor(machine.id)
    cursor.odometer = 99
    expect(log.cursorFor(machine.id).odometer).toBe(1_000)
  })

  it('sổ ghi trước khi có làn tay đọc lên vẫn hợp lệ, hai trường mới bằng 0', async () => {
    const filePath = join(dir, 'production.json')
    await writeFile(filePath, JSON.stringify({
      schemaVersion: PRODUCTION_SCHEMA_VERSION,
      updatedAt: '2026-08-13T00:00:00Z',
      cursors: { [machine.id]: { odometer: 1_000_000, at: '2026-08-13T03:00:00Z' } },
      buckets: {
        '2026-08-13|ca-1|mch-hn-001': {
          date: '2026-08-13', shiftId: 'ca-1', shiftName: 'Ca ngày', machineId: machine.id, siteId: 'hn-1',
          stitches: 50_000, runSeconds: 3_600, readings: 12, resets: 0, anomalies: 0,
          firstAt: '2026-08-13T03:00:00Z', lastAt: '2026-08-13T04:00:00Z',
        },
      },
    }))
    const log = newLog()
    await log.load()
    expect(log.query()[0]).toMatchObject({ stitches: 50_000, manualStitches: 0, readings: 12, manualReadings: 0 })
    // Con trỏ cũ không có `quality`; những mốc đó đều là số của máy, vì làn tay chưa tồn tại.
    expect(log.cursorFor(machine.id)).toMatchObject({ odometer: 1_000_000, quality: 'verified' })
  })
})
